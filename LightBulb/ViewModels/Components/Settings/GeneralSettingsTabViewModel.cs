...
) : SettingsTabViewModelBase(settingsService, localizationManager, 0)
{
    ...
    public double CustomPresetTemperature
    {
        get => SettingsService.CustomPresetConfiguration.Temperature;
        set =>
            SettingsService.CustomPresetConfiguration = new ColorConfiguration(
                value.Clamp(SettingsService.MinimumTemperature, SettingsService.MaximumTemperature),
                CustomPresetBrightness
            );
    }

    public double CustomPresetBrightness
    {
        get => SettingsService.CustomPresetConfiguration.Brightness;
        set =>
            SettingsService.CustomPresetConfiguration = new ColorConfiguration(
                CustomPresetTemperature,
                value.Clamp(SettingsService.MinimumBrightness, SettingsService.MaximumBrightness)
            );
    }
    ...
    public bool IsBreakReminderEnabled
    {
        get => SettingsService.IsBreakReminderEnabled;
        set => SettingsService.IsBreakReminderEnabled = value;
    }

    public double BreakWorkDurationMinutes
    {
        get => SettingsService.BreakWorkDuration.TotalMinutes;
        set => SettingsService.BreakWorkDuration = TimeSpan.FromMinutes(value.Clamp(5, 240));
    }

    public double BreakDurationSeconds
    {
        get => SettingsService.BreakDuration.TotalSeconds;
        set => SettingsService.BreakDuration = TimeSpan.FromSeconds(value.Clamp(5, 300));
    }
}

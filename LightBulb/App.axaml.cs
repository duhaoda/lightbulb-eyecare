...
public partial class App : Application, IDisposable
{
    ...
    private Window? _breakOverlayWindow;
    ...
    public App()
    {
        ...
        services.AddTransient<BreakOverlayViewModel>();
        ...
        _breakReminderService = _services.GetRequiredService<BreakReminderService>();
        _mainViewModel = _services.GetRequiredService<ViewModelManager>().GetMainViewModel();

        // Show the break reminder overlay whenever a break becomes due
        _breakReminderService.BreakStarted += (_, _) => ShowBreakOverlay();
        _breakReminderService.BreakEnded += (_, _) => HideBreakOverlay();
    ...
    {
        // Re-use the overlay if it's already shown
        if (_breakOverlayWindow is { } existingWindow)
        {
            existingWindow.ShowActivateFocus();
            return;
        }
        ...
        var window = viewManager.TryBindWindow(viewModelManager.GetBreakOverlayViewModel());
        if (window is null)
            return;

        _breakOverlayWindow = window;
        window.Closed += (_, _) => _breakOverlayWindow = null;
        window.Show();
        window.Activate();
    }

    private void HideBreakOverlay()
    {
        _breakOverlayWindow?.Close();
        _breakOverlayWindow = null;
    }
